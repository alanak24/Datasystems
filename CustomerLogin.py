...
def give_feedback(user_id, laptop):
    global screen12
    screen12 = Toplevel(screen)
    screen12.title("Give Feedback")
    screen12.geometry("400x300")
    
    global rating_var, comment_var
    rating_var = StringVar()
    comment_var = StringVar()

    Label(screen12, text=f"Feedback for {laptop[1]}").pack()
    Label(screen12, text="Rating (1-5)").pack()
    rating_entry = Entry(screen12, textvariable=rating_var)
    rating_entry.pack()
    Label(screen12, text="Comment").pack()
    comment_entry = Entry(screen12, textvariable=comment_var)
    comment_entry.pack()
    
    Button(screen12, text="Submit Feedback", command=lambda: submit_feedback(user_id, laptop[0])).pack()
def submit_feedback(user_id, laptop_id):
    rating = rating_var.get()
    comment = comment_var.get()
    now = datetime.now()

    try:
        cursor.execute("INSERT INTO review (Laptop_ID, User_ID, Purchase_History_ID, Review_Rating, Review_Comment, Review_Date) VALUES (%s, %s, NULL, %s, %s, %s)", 
                       (laptop_id, user_id, rating, comment, now))
        db.commit()
